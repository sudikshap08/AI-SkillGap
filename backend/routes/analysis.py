...
from ..services.llm_service import generate_explanation, generate_personalized_guidance
...
def make_analysis(db,user_id,role_id,skills):
    ...
    guidance=generate_personalized_guidance(role.name,score,results,roadmap,projects)
    ...
    return {"analysis_id":analysis.id,"role":role.name,"score":score,"strong":strong,"improvement":improvement,"missing":missing,"gaps":results,
      ...
      "skill_map":{
        "strong":[{"skill":r["skill_name"],"importance":r["importance"]} for r in results if r["status"]=="Strong"],
        "improvement":[{"skill":r["skill_name"],"importance":r["importance"]} for r in results if r["status"]=="Improvement"],
        "missing":[{"skill":r["skill_name"],"importance":r["importance"]} for r in results if r["status"]=="Missing"]
      },
      "ai_guidance":guidance,
      "projects":[{"title":p.title,"description":p.description,"difficulty":p.difficulty,"github_url":p.github_url,"skills":[ps.skill.name for ps in p.project_skills],"ai_reason":guidance["project_reasons"].get(p.title)} for p in projects],"explanation":explanation}
